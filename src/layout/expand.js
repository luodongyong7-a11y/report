...
import { resolveBandEdges } from './bands.js'
...
/**
 * 标签模式驱动数据集:元素文档序中第一个 `${dsN.*}` 且能解成数组的路径。
 * 无数组绑定则返回 null(整页静态标签,1 页)。
 */
function resolveLabelDriver (elements, dataset) {
  for (const element of elements) {
    const content = element.content || ''
    const tokens = content.match(/\$\{([^}]+)\}/g) || []
    for (const raw of tokens) {
      const token = raw.slice(2, -1).trim()
      const pathForArray = getIterationArrayPath(token, dataset)
      if (!pathForArray) continue
      const rows = resolveIterableArrayFromDataset(dataset, pathForArray)
      if (Array.isArray(rows)) return { path: pathForArray, rows }
    }
  }
  return null
}

function collectAggregates (elements, sumSet, subtotalSet) {
  for (const element of elements) {
    const content = element.content || ''
...
function expandLabelElements (templateData) {
  const dataset = templateData.dataset || {}
  const elements = templateData.elements || []
  const renderedElements = []
  const sumSet = new Set()
  const subtotalSet = new Set()
  collectAggregates(elements, sumSet, subtotalSet)

  const driver = resolveLabelDriver(elements, dataset)
  if (driver && driver.rows.length === 0) {
    return { renderedElements, sumSet, subtotalSet }
  }
  const items = driver ? driver.rows : [undefined]
  const pathForArray = driver ? driver.path : null

  const pushEl = (element, index, replacedContent, expressionPath, rowItem, rowBg, previewNullishWarning) => {
  ...
  for (let index = 0; index < items.length; index++) {
    ...
    for (const element of elements) {
      ...
      pushEl(
        ...
        index,
        replacedContent,
        own ? own.path : pathForArray,
        item,
        rowBg,
        previewNullishWarning
      )
    }
  }
...
export function expandElements (templateData) {
  if (isLabelPrintKind(templateData)) {
    return expandLabelElements(templateData)
  }
  const dataset = templateData.dataset || {}
  const elements = templateData.elements || []
  const renderedElements = []
  const sumSet = new Set()
  const subtotalSet = new Set()
  const bands = resolveBandEdges(templateData)
  const headerY = bands.headerY
  const summaryA = bands.summaryA
  const summaryB = bands.summaryB
  const footerY = bands.footerY
  const groupIter = buildGroupArrayIteration(
    elements, dataset,
    headerY, summaryA, summaryB, footerY
  )
  ...
  for (const element of elements) {
    ...
    const inDataArea = element.y >= headerY && element.y < summaryA
    // 汇总B→页脚:与数据区一样可数组展开 + 自增高(group/prev 级联在 flow 中处理)。
    // summaryEnabled=false 时 summaryB===footerY，SBF 带为空。
    const inSbfArea = typeof summaryB === 'number' && typeof footerY === 'number' &&
      summaryB < footerY &&

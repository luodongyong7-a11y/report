...
//
// printKind==='label' 时走 flow-label:整页按数据集行克隆,一行一页,元素钉设计坐标(不堆行、不 autoGrow)。
//
...
import { buildLabelFlowModel } from './flow-label.js'
import {
...
export function buildFlowModel (templateData, { measureCell } = {}) {
  ...
  if (isLabelPrintKind(templateData)) {
    return buildLabelFlowModel(templateData, { measureCell })
  }
  ...
  const summaryB = bands.summaryB
  const footerY = bands.footerY
  const summaryBandHeight = bands.summaryEnabled
    ? Math.max(0, (summaryB ?? footerY) - (summaryA ?? footerY))
    : 0
  ...
  for (const el of renderedElements) {
    ...
    else if (bands.summaryEnabled && y >= summaryB && y < footerY) sbfEls.push(el)
    else if (bands.summaryEnabled && y >= summaryA && y < summaryB) summaryEls.push(el)
  ...
  // 分批打印:stream.forcedTotalPages / pageOffset / forcedSums 与全量口径对齐。
  ...
  // totalPages = 本批实际页数(供分批累加 pageOffset);页脚「共 y 页」用 displayTotalPages。
  const totalPages = pageSpecs.length
  const displayTotalPages = (Number.isFinite(forcedTotal) && forcedTotal > 0)
    ? Math.floor(forcedTotal)
    : totalPages
  const forcedSums = (stream.forcedSums && typeof stream.forcedSums === 'object' && !Array.isArray(stream.forcedSums))
    ? stream.forcedSums
    : null
  if (forcedSums) {
    for (const [inner, val] of Object.entries(forcedSums)) {
      if (val == null || val === '') continue
      try {
        sumMap.set(inner, val instanceof Decimal ? val : new Decimal(val))
      } catch {
        /* keep measured sum if inject invalid */
      }
    }
  }
  ...
  const resolveCellContent = (cell, pageIndex, subtotalMap) => {
    ...
    if (content.includes('${page}') || content.includes('${total}')) {
      ...
      if (content.includes('${total}')) updated = updated.replace(/\$\{total\}/g, displayTotalPages)
